#-------------------------------------------------------------------------------
# Name:        "A HEWLETT-PACKARD 86120B Multi-Wavelength Meter"
# Purpose:     WL Test in ShangHai Lab           
#
...
class Wavemeter:
    ...
    #def __init__(self, host = "172.29.150.17", port = 1234, addr = "11", sock = None):
        self.host_ = host
        self.port_ = port
        self.addr_ = addr
        self.logFile = "NA"
        self.printLog("Connecting to Wavemeter...")
...
#				self.printLog("Error!! Connect to Wavemeter fail")
    ...
    def getWL(self):
        WL = self.ask(":MEAS:SCAL:POW:WAV?")
        return WL    
    
    def getFreq(self):
		Freq = self.ask(":MEASure:SCALar:POWer:FREQuency?")
		#Freq=re.findall(r"+(.*)\\n", Freq)
		Freq=Freq.replace("\n","")
		Freq=float(Freq)*1e-9 #GHz
		return Freq  
    
    def getPower(self):
		Power = self.ask(":MEASure:SCALar:POWer?")
		Power=Power.replace("\n","")
		Power=float(Power)
		return Power  
#s.sock.send("++addr 11\n".encode())
