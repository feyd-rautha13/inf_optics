...
startTime = time.time()
logPath = ".\\results\\{0}_{1}_{2}_{3}".format("20C_Steady_T-3_BAX",Vendor,SN,curTime)
...
T=-3 #tilt set
hal.write("hw 1 ofp2 2 c x SetTilt EDFA_2 " + str(T*100))
# hw 1 ofp2 2 c x SetTilt EDFA_2 -300
Pin=[-10, 10.8, -13, 7.8, -16, 4.8, -19, 1.8, -22, -1.2]
Gain=[10, 10, 13, 13, 16, 16, 19, 19, 22, 22]
NFspec0=[9.6, 9.6, 7.3, 7.3, 6.1, 6.1, 5.5, 5.5, 5.2, 5.2]
NFcorr=[0.9, 0.9, 0.74, 0.74, 0.58, 0.58, 0.41, 0.41, 0.25,0.25]
NFspec=[]
for i in range(10):
    NFsp=NFspec0[i]+NFcorr[i]*abs(T)
    NFspec.append(NFsp) 
...
#    printLog("Pre-amp Power:" + str(A))
    ...
    osa.write("WNFAN")
    C=osa.query("ANA?",flag=b"\r\n", endnumber=2).decode('utf8').rstrip().split(",")
...
#    GA=Gm-Gain[i]
    ...
    plt.savefig(logPath+"\\G{0}_P{1}_T{2}_spectrum.png".format(Gain[i],Pin[i],T), format='png', dpi=96)
    ...
    plt.savefig(logPath+"\\G{0}_P{1}_T{2}.png".format(Gain[i],Pin[i],T), format='png', dpi=96)
...
#    print ("Max/Min Gain (dB)	{0}".format(Gm),file=f)
    ...
    print ("Gain Tilt (dB)	{0}	{1}".format(Tilt,str(T)+"+/-0.5dB"),file=f)
