...
#hal.write("hw 1 cpld w 0xb2 0x82") 
...
startTime = time.time()
logPath = ".\\results\\{0}_{1}_{2}_{3}".format("45C_PD_EDFA1",Vendor,SN,curTime)
os.mkdir(logPath)
...
###PD in test
hal.write("hw 1 ofp2 2 x Period ALL_ACTION 0")
...
#    PoutList.append(Pout)
    ...
    iqs.write(Att_cmd)
    time.sleep(4)
    hal.write("hw 1 ofp2 2 c x GetPower PD3")
...
for i in range(20):
    ...
    Pout=18-i*2 #dBm
    PoutList.append(Pout)
    Pout_cmd="hw 1 ofp2 2 c x SetPowerSetting EDFA_1 " + str(Pout*100) #mdBm
    hal.write(Pout_cmd)
    hal.read_raw()
    time.sleep(0.5)
    ...
    hal.write("hw 1 ofp2 2 c x GetPower PD4")
    data=hal.read_raw().decode()
    data = re.split(r'\r|\n', data)
