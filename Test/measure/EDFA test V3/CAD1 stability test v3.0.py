...
startTime = time.time()
logPath = ".\\results\\{0}_{1}_{2}_{3}".format("-5C_Stability_EDFA1",Vendor,SN,curTime)
os.mkdir(logPath)
...
#for i in range(len(Pin)):
    ...
    G_cmd="hw 1 ofp2 2 c x SetGainSetting EDFA_1 " + str(Gain[i]*100)
...
Po=-4 
Pout_cmd="hw 1 ofp2 2 c x SetPowerSetting EDFA_1 " + str(Po*100) #mdBm
hal.write(Pout_cmd)
print("test start APC")
...
plt.ylabel("Pout")
plt.grid()
plt.savefig(logPath+"\\{0}_P{1}.png".format("APC",Po), format='png', dpi=96)

curTime = datetime.utcnow().strftime("%Y%m%d%H%M")
f = open(logPath+"\\Stability_test_data.log", "a+")
print ("SUMMARY RESULTS:",file=f)
print ("======================",file=f)
print ("Output Power (dBm)	{0}".format(Po),file=f)
print ("----------------",file=f)
print ("Gain Stablity (dB)	{0}	{1}".format(GS,"+/-0.2dB"),file=f)
print ("----------------",file=f)
print ("{0}	{1}".format("Count","Pout"), file=f)
